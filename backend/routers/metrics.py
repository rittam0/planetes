...
_metrics = {
    ...
    "data_counts": {"live_satellites": 0, "nasa_asteroids": 0, "synthetic_objects": 0, "sgp4_propagated": 0, "llm_success": 0, "llm_fallback": 0},
    ...
    return {
        ...
        "external_apis": {
            ...
            "groq": {
                "status": (
                    "not_configured"
                    if not os.getenv("GROQ_API_KEY") or os.getenv("GROQ_API_KEY") == "DEMO_KEY"
                    else "connected"
                    if _metrics["data_counts"]["llm_success"] > 0
                    else "degraded"
                ),
                "avg_latency_ms": groq_avg,
                "calls": _metrics["latency_ms"]["groq_count"],
                "failures": _metrics["errors"]["groq_failures"],
                "llm_success_rate_pct": llm_rate,
                "llm_success_count": _metrics["data_counts"]["llm_success"],
        ...
        "version": "1.0.0",
        "data_counts": _metrics["data_counts"],
        "architecture": "FastAPI + SGP4 + NASA NeoWs + KeepTrack + Groq + LangGraph stateful investigation workflow",
    }
