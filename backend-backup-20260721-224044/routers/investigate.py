...
async def investigate(req: InvestigateRequest):
    ...
    try:
        llm_start = time.time()
        llm_report = await generate_report(
            req.conjunction_id,
            risk,
            object_a,
            object_b,
        ...
        print(f"[Investigate] LLM failed: {e}")
    
    if llm_report and "risk_summary" in llm_report:
        report = llm_report["risk_summary"]
        recommendation = llm_report.get("recommended_action", "MONITOR")
        llm_enabled = True
    else:
        if level == "HIGH":
    ...
    return {
        ...
        "agents": [
            ...
            },
            {
                ...
                "action": "LLM-structured report via Groq" if llm_enabled else "Deterministic template fallback",
                "result": report,
                "latency_ms": llm_latency if llm_enabled else 0,
        ...
        "architecture": "Three-step pipeline: Data Fetch -> SGP4 Risk Analysis -> LLM Report Generation (Groq + Fallback)"
    }
